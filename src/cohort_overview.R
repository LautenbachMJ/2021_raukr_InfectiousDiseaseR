...
## understanding data
max(dummy.data$visit)
as_tibble(dummy.data) %>% filter(patient_id == "81") %>% head(50)


param_mapping <- c("param1" = "CD4","param2" = "CD8","param3" = "Viral_load")
parsed_data <- dummy.data

## plot
parsed_data %>% 
  #filter(patient_id == "81") %>%
  ggplot() +
  coord_flip() +
  ...
  labs(title = "Cohort overview",
       subtitle = paste0('# of patients included: ',length(unique(dummy.data$patient_id))),
       y = "Time",
       color = "") +
