...
author: "Jamirah Nazziwa, Ona Deulofeu, Jennie Olofsson, Julius Lautenbach and Zoé Pochon"
...
### 4.1. Parameters levels per day for each patient
...
### 4.2. Parameters levels per day for each patient (solid line and then dashed when under treatment) (Not working yet)
```{r message=FALSE}
plot_param_per_day <- function(data = data, x = days_po, y, category = patient_id, treated = art_status) 
{
ggplot(data, aes({{x}}, {{y}}, group = {{category}}, color = as.factor({{category}}, line_type = as.factor({{treated}})))) +
    geom_smooth(line_type = as.factor({{treated}}), na.rm = TRUE, se = FALSE)
}
...
stat_smooth(aes(linetype = if_else({{treated}} == "yes", "dashed", "solid"))) +
    geom_smooth(na.rm = TRUE, se = FALSE)
